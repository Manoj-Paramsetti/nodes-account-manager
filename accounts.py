...
def user_nav_options(user_input):
    if user_input == "1":
        ...
        os.system("")
        os.system(f'sudo su {username} -c mkdir -p ~/.ssh && echo "{ssh_key}" > /home/{username}/.ssh/authorized_keys && exit')
        ...
        for node in nodes:
            try:
                ...
                os.system(f'ssh ubuntu@{node["ipv4"]} sudo su {username} -c mkdir -p ~/.ssh && echo "{ssh_key}" > /home/{username}/.ssh/authorized_keys ')
                os.system(f'ssh ubuntu@{node["ipv4"]} sudo chown {username}:{username} user/{username}/.ssh/authorized_keys')
            except:
